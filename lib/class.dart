...
class _ClassbodyState extends State<Classbody> {
  ...
  void refreshClasses() {
    db.collection("users").doc(AuthenticationHelper().uid).get().then((value) {
      var x = value.data()?["Students"];
      for (var j in x.entries) {
     //   print(j);
      }

    });

    db.collection("users").doc(AuthenticationHelper().uid).collection("classes").get().then((querySnapshot) {
